...
try {
    // 1. Leer el archivo de prueba (CLI: php Test2.php testD.txt)
    $archivoPrueba = $argv[1] ?? 'archivo5_arreglos_ndim.txt';
    $rutaPrueba = __DIR__ . '/' . $archivoPrueba;
    $input = InputStream::fromPath($rutaPrueba);
    ...
    if ($funcionMain !== null) {
        ...
        \App\Utilities\Salida::$salidasConsola[] = "❌ Error: cLa función 'main' no está declarada en el archivo.";
    }
    //imprimir  la salida en consola
    foreach (Salida::$salidasConsola as $linea) {
        $lineaLimpia = limpiarLineaConsola((string) $linea);
        if ($lineaLimpia === null || $lineaLimpia === '') {
            continue;
        }
        echo $lineaLimpia . "\n";
    }
    ...
    if (count($manejadorErrores->erroresLexicos) > 0) {
        echo "\n❌ ERRORES LÉXICOS:\n";
        foreach ($manejadorErrores->erroresLexicos as $err) {
            $linea = $err['linea'] ?? '?';
            $col = $err['columna'] ?? '?';
            $desc = $err['descripcion'] ?? 'Desconocido';
            echo "  [Línea {$linea}, Columna {$col}] {$desc}\n";
            $totalErrores++;
        }
    }

    if (count($manejadorErrores->erroresSintacticos) > 0) {
        echo "\n❌ ERRORES SINTÁCTICOS:\n";
    ...
    if ($totalErrores === 0) {
        echo "\n✅ Compilación exitosa: sin errores léxicos, sintácticos ni semánticos.\n";
    }
...
/**
 * Limpia trazas de depuracion para que la salida de pruebas sea legible.
 */
function limpiarLineaConsola(string $linea): ?string
{
    $linea = trim($linea);
    if ($linea === '') {
        return null;
    ...
    foreach ($prefijosDebug as $prefijo) {
        if (str_starts_with($linea, $prefijo)) {
            return null;
        }
    }

    if (str_starts_with($linea, '🖨️')) {
        $linea = trim(substr($linea, strlen('🖨️')));
    }
